...
training_set <- oncoth1_transf[train_rows,]
test_set <- oncoth1_transf[test_rows,]
...
for (i in seq_len(nrow(cv_out))) {
  ...
  valid_rows <- cv_info_mapping %>% 
    filter(set == "train", fold == i_fold) %>% 
    pull(row) %>%
    as.integer()
  # Get train row number
  tr_rows <- cv_info_mapping %>% 
    filter(set == "train", fold != i_fold) %>% 
    pull(row) %>%
    as.integer()
  ...
  # Sanity check
  stopifnot(!valid_rows %in% tr_rows)
  
  # Subset data
  valid_data <- training_set[valid_rows,]
  tr_data <- training_set[tr_rows,]
  
  # Sanity check
  stopifnot(
    all(row.names(tr_data) %in% train_rows), 
    all(row.names(valid_data) %in% valid_rows)
  )
  ...
  # X_train <- predict(preproc, tr_data)
  # X_valid <- predict(preproc, valid_data)
