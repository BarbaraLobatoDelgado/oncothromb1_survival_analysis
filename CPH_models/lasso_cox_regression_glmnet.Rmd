...
baseline_lasso <- penalized(
    ...
    unpenalized = ~1,
    lambda1 = 10, # try zero and 0.1. C indices should be ~ 0.8
...
for (row_idx in seq_len(nrow(regpath_results))) {
  ...
  cindex_train <- concordance(y_train ~ lp_train)$concordance
  # Save result
  ...
  cindex_valid <- concordance(y_valid ~ lp_valid)$concordance
  # Save result
