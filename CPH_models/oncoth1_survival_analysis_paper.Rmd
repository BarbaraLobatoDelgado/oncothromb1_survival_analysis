...
```{r}
KM_curve_plots_VTE_another_covariate <- function(clinical_data, survival_data, covariate, patient_group = "patient_group") {
  ...
  # counts <- clinical_data %>%
  #   count({{covariate}}, {{patient_group}})
  
  n_cat1_no_vte <- clinical_data %>% 
    filter({{covariate}} == cat1, {{patient_group}} == "Control") %>% pull(n)
  n_cat1_vte <- clinical_data %>% 
    filter({{covariate}} == cat1, {{patient_group}} == "Case") %>% pull(n)
  n_cat2_no_vte <- clinical_data %>% 
    filter({{covariate}} == cat2, {{patient_group}} == "Control") %>% pull(n)
  n_cat2_vte <- clinical_data %>% 
    filter({{covariate}} == cat2, {{patient_group}} == "Case") %>% pull(n)
