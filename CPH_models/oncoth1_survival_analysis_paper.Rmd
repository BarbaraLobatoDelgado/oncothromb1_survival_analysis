...
This output contains several item:
...
- 0.95LCL and 0.95UCL: The lower and upper 95% confidence limits for the median survival time.


```{r}
# Get information for all time points from Kaplan-Meier estimation
summary_fit = summary(td_surv_fit)
formatted_summary = data.frame(
  yime = summary_fit$time,
...
# kable(formatted_summary, format = "markdown", caption = "Survival Analysis Summary")

formatted_summary %>%
  kbl(caption = "Survival Analysis Summary") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
                full_width = F) %>%
  column_spec(1, bold = TRUE, color = "white", background = "#2E9FDF") %>%
  column_spec(2:8, border_left = TRUE, border_right = TRUE) %>%
  add_header_above(c(" ", "Summary Statistics" = 7))
