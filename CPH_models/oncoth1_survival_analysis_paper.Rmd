...
```{bash}
# sshfs blobato@172.31.60.101:/home/datasets ~/datasets/
```
...
tryCatch({
  ...
  warning = function(w) {
    ...
    load("/mnt/ir-bioinf02/home/blobato/oncothromb01/oncothr1_versions/oncothr1.RData_0.0.8005")
    oncoth1 <- obj
    rm(obj)
  }
)
...
```{r}
table(oncoth1_data$tu_1st_vte_all_days)
table(oncoth1_data$patient_group)
sum(oncoth1_data$tu_1st_vte_all_days < 0, na.rm = TRUE)
table(oncoth1_data$patient_group, oncoth1_data$tu_1st_vte_all_days)
table(oncoth1$data$tu_1st_vte_all_from_cancer_diagnosis_days_corrected)

yy = oncoth1_data[oncoth1_data$tu_1st_vte_all_days < 0, ]
oncoth1_vte_t0 <- oncoth1_data %>% filter(id %in% unique(yy$id))

patient_no_VTE_but_tu1vte <- oncoth1_data %>% filter(patient_group == "Control") %>% filter(tu_1st_vte_all_days != 0)
```

```{r}
oncoth1_cat_conc <- oncoth1_data %>% 
  filter(n_appointment_patient_became_case == "Beginning of study") %>%
  select(id, tu_1st_vte_all_from_cancer_diagnosis_days_corrected)

oncoth1_cat_conc_date <- oncoth1_data %>% 
  filter(tu_1st_vte_all_from_cancer_diagnosis_days_corrected < 1) %>%
  select(id, tu_1st_vte_all_from_cancer_diagnosis_days_corrected)
```
...
```{r}
oncoth1_v5 <- get_project("oncothr1")
```
