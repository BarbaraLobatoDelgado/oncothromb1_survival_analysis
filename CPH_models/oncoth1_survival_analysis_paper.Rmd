...
```{r}
km_estimator_summary = function(km_survfit) {
...
km_estimator_summary(km_estimator_fit)
```
...
km_estimator_pval = get_robscore_pval(data = survival_vte_tmerge, covariates = c("vte_event"))
```
...
km_estimator_fit_stage = survfit(
  Surv(
    time = tstart, 
    time2 = tstop, 
    event = death) ~ 
...
km_estimator_fit_stage
```

```{r}
# Get p-value
km_estimator_pval_vte_stage = get_robscore_pval(survival_vte_tmerge, covariates = c("vte_event", "tnm_stage_two_groups")) 
```
...
#median_survival_days = surv_median(km_estimator_fit_stage)
...
```{r}
summary(km_estimator_fit_stage, times = 18*30.25)
```
...
stratified_primary_tumor_km_plot_vte = 
  ggsurvplot(
...
# Add p-value
stratified_primary_tumor_km_plot_vte$plot = 
  stratified_primary_tumor_km_plot_vte$plot + 
  annotate(
    geom = "text", 
    x = 26,
