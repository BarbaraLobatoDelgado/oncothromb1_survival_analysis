...
survplot_custom = function(survfit_obj, title, n_no_vte, n_vte, logrank_pval, legend_labels, line_colours) {
  ...
  if (logrank_pval <= 0.05 && logrank_pval > 0.01) {
...
```{r}
# Fit Kaplan-Meier estimator for men
km_fit_vte_male <- survfit(Surv(
  time = tstart, 
  time2 = tstop, 
  event = death) ~ vte_event + gender + cluster(id),
  id = id,
  data = survival_vte_tmerge %>% filter(gender == "Male"))

km_fit_vte_male
```
```{r}
# Fit Kaplan-Meier estimator for men
km_fit_vte_female <- survfit(Surv(
  time = tstart, 
  time2 = tstop, 
  event = death) ~ vte_event + gender + cluster(id),
  id = id,
  data = survival_vte_tmerge %>% filter(gender == "Female"))
...
```{r}
# Get p-value for VTE stratified by sex
km_pval_vte_sex <- get_robscore_pval(data = survival_vte_tmerge, covariates = c("vte_event", "gender"))

# Get p-value for KM curve with VTE only for men
km_pval_vte_sex_male <- get_robscore_pval(data = survival_vte_tmerge %>% filter(gender == "Male"), covariates = c("vte_event", "gender"))

# Get p-value for KM curve with VTE only for men
km_pval_vte_sex_female <- get_robscore_pval(data = survival_vte_tmerge %>% filter(gender == "Female"), covariates = c("vte_event", "gender"))
```
...
#   
#   # Get categories of covariate
#   cat1 <- levels(clinical_data[[covariate]])[1]
#   cat2 <- levels(clinical_data[[covariate]])[2]
#   
#   print(cat1)
#   print(cat2)
#   
...
#   n_cat1_no_vte <- clinical_data %>% 
#     filter(.data[[covariate]] == cat1, .data[[patient_group]] == "Control") %>% nrow()
#   n_cat1_vte <- clinical_data %>% 
#     filter(.data[[covariate]] == cat1, .data[[patient_group]] == "Case") %>% nrow()
#   n_cat2_no_vte <- clinical_data %>% 
#     filter(.data[[covariate]] == cat2, .data[[patient_group]] == "Control") %>% nrow()
#   n_cat2_vte <- clinical_data %>% 
#     filter(.data[[covariate]] == cat2, .data[[patient_group]] == "Case") %>% nrow()
#   
#   # print(counts)
#   print(n_cat1_no_vte)
#   print(n_cat2_no_vte)
...
#   
#   # KM fit
#   km_fit_VTE_cat1 <- survfit(km_formula,
#     id = id,
#     data = survival_data %>% filter(.data[[covariate]] == cat1))
#   
#   # Get p-value for KM curve with VTE only for men
#   km_pval_vte_cat1 <- get_robscore_pval(data = survival_data %>% filter(.data[[covariate]] == cat1),
#                                             covariates = c(VTE, covariate))
#   
#   # KM plot 1
#   km_plot_VTE_cat1 = survplot_custom(
#     km_fit_VTE_cat1, 
#     title = cat1, 
#     n_no_vte = n_cat1_no_vte, 
#     n_vte = n_cat1_vte, 
#     legend_labels = c("No VTE", "VTE"), 
#     line_colours = c("black", "red"),
#     logrank_pval = km_pval_vte_cat1) + 
...
```{r}
KM_curve_plots_VTE_another_covariate(
...
```{r}
# KM curve of VTE for men
km_plot_vte_male = survplot_custom(
  km_fit_vte_male, 
  title = "Male", 
  n_no_vte = n_male_no_vte, 
  n_vte = n_male_vte, 
  legend_labels = c("No VTE", "VTE"),
  line_colours = c("black", "red"),
  logrank_pval = fit1_man_log_rank_test_pval
  ) + 
  theme(legend.position = "none")
...
```{r}
# jpeg("../graphs/paper/KM_female.jpeg", width = 8, height = 4, units = 'in', res = 700)

km_plot_vte_female = survplot_custom(
  km_fit_vte_female, 
  title = "Female", 
  n_no_vte = n_female_no_vte, 
  n_vte = n_female_vte, 
  legend_labels = c("No VTE", "VTE"),
  line_colours = c("black", "red"),
  logrank_pval = fit1_woman_log_rank_test_pval
  ) + 
  theme(legend.position = "none", 
...
```{r}
# #jpeg("./graphs/time_dependent_covariate_analysis/vte_td_KM_curve_gender_conf_int.jpeg", width = 8, height = 4, units = 'in', res = 700)
# 
# n_man = as.numeric(sum(oncoth1[["data"]]$gender == "Male"))
# n_woman = as.numeric(sum(oncoth1[["data"]]$gender == "Female"))
# 
# # A data frame with labels for each facet
# f_labels_gender <- data.frame(gender = c("Male", "Female"), 
#                               label = c(
#                                 paste0("n=", n_man), 
#                                 paste0("n=", n_woman)))
# 
# f_labels_gender_logrank_pval = data.frame(gender = c("Male", "Female"), 
#                                    label = c(km_pval_vte_sex_male, 
#                                              km_pval_vte_sex_female))
# 
...
#     y = 0.12, 
#     aes(label = label), 
#     data = f_labels_gender_logrank_pval)
# 
# #dev.off()
```
...
KM_curve_VTE_sex_combined_plot <- cowplot::plot_grid(
  ...
  align = "h",
  ncol = 2, 
  rel_heights = c(0.7, 0.7),  # Adjust heights to fit both plots
