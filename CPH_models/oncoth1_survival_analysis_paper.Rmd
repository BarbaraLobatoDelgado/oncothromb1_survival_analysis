...
stepwise_folds <- cut(folds_shuffled_ids, breaks = k, labels = FALSE)
```
...
for (i in 1:k) {
  ...
  stepwise_model = StepReg::stepwiseCox(
    ...
    gender +
    bmi_value +
    # performance_status_category_corrected_imp + # Multicollinearity with cancer type, tobacco use, tumor resection...
    ...
    arterial_hypertension +
    venous_insufficiency_imp + # High GVIF, multicollinearity in some models
    ...
    # family_background_vte + # High GVIF, multicollinearity with venous insufficiency in some folds
    previous_vte +
    previous_ate +
    anticoag_tx_currently +
    antiaggreg_tx_currently, 
    # blood_transfusion + # Correlated to pancreatic cancer and EPO
