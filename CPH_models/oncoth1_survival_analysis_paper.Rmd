...
```{r}
# Calculation of sample size to have statistical power 80%
sample_size_CPH <- ssizeEpi(
  X1 = survival_data$censored_patient, 
  X2 = ifelse(survival_data$patient_group == "Case", 1, 0), 
  failureFlag = 1, 
  power = 0.8, 
...
```{r}
# univariate_km_log_rank_test = function(tu_var, event_var, dataset, detailed_summary = FALSE, plot_curve = TRUE) {
#   
#   # Make sure inputs are entered properly
#   if (!(tu_var %in% names(dataset) && event_var %in% names(dataset))) {
#     stop("The specified variables are not present in the dataset.")
#   }
#   
#   # Define survival object
#   surv_object = Surv(time = dataset[[tu_var]], event = dataset[[event_var]])
# 
...
#   survfit_object = survfit(formula = surv_object ~ 1, data = dataset)
#   
#   # Show summary of survival function (in months)
#   cat("Summary of KM fitted survival curve:")
#   cat("\n")
...
#   cat("\n")
#   time_in_months = 18 # Use consistent time units
#   print(summary(survfit_object, times = time_in_months * 30.25))
#   cat("\n")
#   
#   
#   # Summary in df, with nicer format (in days)
#   if (detailed_summary) {
#     cat("Nice summary of KM fitted survival curve:")
#     cat("\n")
#     print(surv_summary(survfit_object, data = dataset))
#     cat("\n")
#   }
#   
#   # Since no curves are being compare, log-rank test cannot be applied
#   cat("There is no stratification of survival estimates. Log-rank test cannot be applied.")
#   
#   # Return survfit object, needed to plot the survival curve
#   if (plot_curve) {
# 
#     km_plot = ggsurvplot(
#       fit = surv_fit(formula = surv_object ~ 1, data = dataset),  
#       data = dataset,
#       break.time.by = 60,
#       censor = FALSE,
#       palette = "black",
