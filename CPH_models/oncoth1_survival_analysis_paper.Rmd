...
```{r}
# Do KM fit of survival and log rank test for all patients in oncothromb1
surv_curve_oncoth1 = univariate_km_log_rank_test(
  tu_var = "follow_up_length_from_cancer_diagnosis_days", 
  event_var = "censored_patient", 
  dataset = oncoth1$data, 
  # detailed_summary = TRUE,
  plot_curve = TRUE
  )

surv_curve_oncoth1$plot
```
...
```{r, include=FALSE}
# Fit Cox model to work around the problem of survfit() not accepting cluster(id)
cox_fit <- coxph(
  Surv(tstart, tstop, death) ~ vte_event + gender + cluster(id),
  data = survival_vte_tmerge
)

# Fit Kaplan-Meier estimator
km_fit_vte_sex <- survfit(cox_fit)
...
cph_fit1 = coxph(Surv(time = tstart, time2 = tstop, event = death) ~ vte_event + gender, 
    data = survival_vte_tmerge)

cph_tests_summary_fit1 = summary(cph_fit1)

#cph_tests_summary_fit1

fit1_log_rank_test_pval = format(
  cph_tests_summary_fit1$sctest["pvalue"], 
  digits = 2, 
  scientific = TRUE)


# # Perform log-rank test and obtain p-value
# logrank_test <- survdiff(Surv(time = tstart, time2 = tstop, event = death) ~ vte_event + gender, 
#     data = td_survival_analysis)
...
```{r, include=FALSE}
# Calculate separate p-values for different values of gender

# Men
man_td_survival_analysis = survival_vte_tmerge %>%
  filter(gender == "Male") 

cph_tests_summary_fit1_man = summary(coxph(Surv(time = tstart, time2 = tstop, event = death) ~ vte_event + gender, 
    data = man_td_survival_analysis))

print(cph_tests_summary_fit1_man)

fit1_man_log_rank_test_pval = format(
  cph_tests_summary_fit1_man$sctest["pvalue"], 
  digits = 2, 
  scientific = TRUE)


# Women
woman_td_survival_analysis = survival_vte_tmerge %>%
  filter(gender == "Female") 

cph_tests_summary_fit1_woman = summary(coxph(Surv(time = tstart, time2 = tstop, event = death) ~ vte_event + gender, 
    data = woman_td_survival_analysis))

print(cph_tests_summary_fit1_woman)

fit1_woman_log_rank_test_pval = format(
  cph_tests_summary_fit1_woman$sctest["pvalue"], 
  digits = 2, 
  scientific = TRUE)
...
f_labels_gender <- data.frame(gender = c("Man", "Woman"), 
                              label = c(
                                paste0("n=", n_man), 
                                paste0("n=", n_woman)))

f_labels_gender_logrank_pval = data.frame(gender = c("Man", "Woman"), 
                                   label = c(fit1_man_log_rank_test_pval, 
                                             fit1_woman_log_rank_test_pval))
