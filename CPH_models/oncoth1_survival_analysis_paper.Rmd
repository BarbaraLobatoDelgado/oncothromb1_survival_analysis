...
```{r}
# Load DB
oncoth1 = get_project("oncothr1", version = "0.0.8001")
```

```{r}
table(oncoth1$data$patient_group)
```


```{r}
table(oncoth1$data$patient_group, oncoth1$data$vte_recurrence)
```
...
comorbidities_cols_index = which(colnames(oncoth1$data) %in% c(
  "diabetes_mellitus", 
  "dyslipidemia", 
  # "tobacco_use_imp", # it is not considered an illness
  "copd", 
...
oncoth1$data$n_comorbidities <- rowSums(oncoth1$data[, comorbidities_cols_index] == "Yes")

oncoth1$data %<>%
  mutate(n_comorbidities = case_when(
    n_comorbidities == 0 ~ "0",
    n_comorbidities == 1 ~ "1",
    n_comorbidities == 2 ~ "2+",
    n_comorbidities == 3 ~ "2+",
    n_comorbidities == 4 ~ "2+",
    n_comorbidities == 5 ~ "2+",
    n_comorbidities == 6 ~ "2+"), .after = venous_insufficiency) %>%
  mutate(n_comorbidities = as.factor(n_comorbidities)) 
...
# Create survival data sets


## Time-dependent data set

```{r}
covariates_df = oncoth1$data %>%
  select(id, 
         patient_group,
         age_when_cancer_dx,
         age_quartiles,
...
survival_data = oncoth1$data %>%
  select(id, 
         ...
         tu_5th_vte_all_from_cancer_diagnosis_days_corrected, 
         tu_5th_vte_all_from_cancer_diagnosis_months_corrected)

#survival_vte_tmerge %<>%
#  mutate_at(vars(starts_with("tu_")), ~ as.numeric(.x))
...
survival_vte_tmerge = tmerge(
  ...
  vte_event = tdc(tu_5th_vte_all_from_cancer_diagnosis_days_corrected)
  )
```
...
survival_vte_tmerge = tmerge(
  ...
  #major_trauma = tdc(tu_1st_major_trauma_from_cancer_diagnosis_days), 
...
```{r}
#survival_vte_tmerge$vte_event = factor(x = survival_vte_tmerge$vte_event, 
                                    #levels = c(0, 1), 
                                    #labels = c("No", "Yes"))
```


## CPH regression data set

```{r}
#survival_dataset = merge(
#  oncoth1$data %>% 
#    select(id, follow_up_length_from_cancer_diagnosis_days, censored_patient),
#  covariates_df, 
#  by = "id"
#)
```
...
```{r}
# Fit survival analysis model
stratified_stage_td_surv_fit = survfit(
  Surv(
    time = tstart, 
    time2 = tstop, 
    event = death) ~ 
...
n_stages_i_ii_iii_no_vte = sum(
  oncoth1$data$tnm_stage_two_groups == "I-II-III" & 
    oncoth1$data$patient_group == "Control")

n_stages_i_ii_iii_vte = sum(
  oncoth1$data$tnm_stage_two_groups == "I-II-III" & 
    oncoth1$data$patient_group == "Case")

n_stages_iv_no_vte = sum(
  oncoth1$data$tnm_stage_two_groups == "IV" & 
    oncoth1$data$patient_group == "Control")
...
#median_survival_days = surv_median(stratified_stage_td_surv_fit)
...
```{r}
summary(stratified_stage_td_surv_fit, times = 18*30.25)
```
...
```{r}
# Fit survival analysis model
stratified_primary_tumor_td_surv_fit = survfit(
  Surv(
    time = tstart, 
    time2 = tstop, 
    event = death) ~ 
...
stratified_primary_tumor_td_surv_fit
```
...
stratified_primary_tumor_td_survival_km_plot = 
  ggsurvplot(
    fit = stratified_primary_tumor_td_surv_fit,
    data = survival_vte_tmerge,
    xscale = 30,
    break.time.by = 60,
    censor = FALSE,
...
n_colorectal_no_vte = sum(
  oncoth1$data$primary_tumor_simplified == "Colorectal" & 
    oncoth1$data$patient_group == "Control")
...
n_nsclc_no_vte = sum(
  oncoth1$data$primary_tumor_simplified == "NSCLC" & 
    oncoth1$data$patient_group == "Control")
...
n_oesophagogastric_no_vte = sum(
  oncoth1$data$primary_tumor_simplified == "Oesophago-gastric" & 
    oncoth1$data$patient_group == "Control")
...
n_pancreatic_no_vte = sum(
  oncoth1$data$primary_tumor_simplified == "Pancreatic" & 
    oncoth1$data$patient_group == "Control")
...
```{r}
# Fit survival analysis model
blood_transf_td_surv_fit = survfit(
  Surv(
    time = tstart, 
    time2 = tstop, 
    event = death) ~ 
...
```{r}
# Fit survival analysis model
immobilisation_td_surv_fit = survfit(
  Surv(
    time = tstart, 
    time2 = tstop, 
    event = death) ~ 
...
immobilisation_td_survival_km_plot = ggsurvplot(
    fit = immobilisation_td_surv_fit,
    data = survival_vte_tmerge,
    xscale = 30,
    break.time.by = 60,
    censor = FALSE,
...
```{r}
# Fit survival analysis model
recent_major_surgery_td_surv_fit = survfit(
  Surv(
    time = tstart, 
    time2 = tstop, 
    event = death) ~ 
...
recent_major_surgery_td_surv_fit
```
...
recent_major_surgery_td_survival_km_plot = ggsurvplot(
    fit = recent_major_surgery_td_surv_fit,
    data = survival_vte_tmerge,
    xscale = 30,
    break.time.by = 60,
    censor = FALSE,
...
```{r}
# Fit survival analysis model
thromboprophylaxis_hospital_td_surv_fit = survfit(
  Surv(
    time = tstart, 
    time2 = tstop, 
    event = death) ~ 
...
thromboprophylaxis_hospital_td_surv_fit
```
...
thromboprophylaxis_hospital_td_survival_km_plot = 
  ggsurvplot(
    fit = thromboprophylaxis_hospital_td_surv_fit,
    data = survival_vte_tmerge,
    xscale = 30,
    break.time.by = 60,
    censor = FALSE,
...
```{r}
# Fit survival analysis model
erythropoietin_tx_td_surv_fit = survfit(
  Surv(
    time = tstart, 
    time2 = tstop, 
    event = death) ~ 
...
erythropoietin_tx_td_survival_km_plot = 
  ggsurvplot(
    fit = erythropoietin_tx_td_surv_fit,
    data = survival_vte_tmerge,
    xscale = 30,
    break.time.by = 60,
    censor = FALSE,
...
```{r}
# Fit survival analysis model
stratified_stage_erythropoietin_tx_td_surv_fit = survfit(
  Surv(
    time = tstart, 
    time2 = tstop, 
    event = death) ~ 
...
stratified_stage_erythropoietin_tx_td_surv_fit
```
...
stratified_stage_erythropoietin_tx_td_survival_km_plot = 
  ggsurvplot(
    fit = stratified_stage_erythropoietin_tx_td_surv_fit,
    data = survival_vte_tmerge,
    xscale = 30,
    break.time.by = 60,
    censor = FALSE,
...
## Backward feature selection in CPH regression model

```{r}
# backward_fs_cph = riskRegression::selectCox(
#   Surv(time = tstart, time2 = tstop, event = death) ~ 
#     cluster(id) + 
#     vte_event + 
#     primary_tumor_simplified + 
...
#     performance_status_category_corrected_imp + 
#     #albumin_imp + 
#     #albumin_imp_category + # High albumin is collinear with something
#     #aptt_imp + 
#     aptt_imp_category +
#     #aptt_ratio_imp + 
#     #bilirubin_imp + 
...
#     #antiaggreg_tx_other_causes + 
#     #antiaggreg_tx_currently
#     blood_transfusion + # Correlated with low blood cells, low hemoglobin, etc.
#     immobilisation + # Correlated with PS
#     #recent_major_surgery + 
#     thromboprophylaxis_hospital + 
#     erythropoietin_tx, # Correlated with low red blood cells
...
```{r}
# Replicate model obtained by backward feature selection to evaluate PH
#cox.zph(coxph(
#  Surv(time = tstart, time2 = tstop, event = death) ~
#    cluster(id) + 
#    vte_event + 
...
#    immobilisation,
#  id = id,
#  data = survival_vte_tmerge), terms = FALSE)
```
...
```{r}
survival_vte_tmerge_dummies = fastDummies::dummy_cols(survival_vte_tmerge)
```


```{r}
stepwise_var_list = list(
  "vte_event", 
  #"`age_when_cancer_dx_50_Younger than 50 years old`",
  #"`age_when_cancer_dx_50_Older or 50 years old`",
  "`age_when_cancer_dx_65_Younger than 65 years old`",
  "`age_when_cancer_dx_65_Older or 65 years old`", 
  #"gender_Male", 
  #"gender_Female",                            
  "menopausal_status_imp_Male", 
  ...
  "bmi_category_Obese", 
  "performance_status_category_corrected_imp_0",
  "performance_status_category_corrected_imp_1", 
  "performance_status_category_corrected_imp_2", 
  "albumin_imp_category_Normal", 
  "albumin_imp_category_Low", 
  "albumin_imp_category_High",  
  "aptt_imp_category_Normal", 
  "aptt_imp_category_Low", 
  "aptt_imp_category_High",
  "aptt_ratio_imp_category_Normal", 
  "aptt_ratio_imp_category_Low",
  "aptt_ratio_imp_category_High", 
  "bilirubin_imp_category_Normal",
  "bilirubin_imp_category_Low", 
  "bilirubin_imp_category_High", 
  "creatinine_imp_category_Normal", 
  "creatinine_imp_category_Low", 
  "creatinine_imp_category_High",
  "alkaline_phosphatase_imp_category_Normal", 
  "alkaline_phosphatase_imp_category_Low", 
  "alkaline_phosphatase_imp_category_High", 
  "hemoglobin_category_Normal", 
  "hemoglobin_category_Low",
  "hemoglobin_category_High", 
  "`inr_imp_category_Normal coagulation`", 
  "`inr_imp_category_Fast coagulation`", 
  "`inr_imp_category_Slow coagulation`",
  "leukocytes_imp_category_Normal", 
  "leukocytes_imp_category_Low",
  "leukocytes_imp_category_High", 
  #"platelets_category_Normal", 
  #"platelets_category_Low", 
  #"platelets_category_High", 
  #"platelets_category_NA", 
  "leukocytosis_imp_No",  
  "leukocytosis_imp_Yes", 
  "thrombocytosis_No",
  "thrombocytosis_Yes", 
  "low_hemoglobin_No", 
  "low_hemoglobin_Yes", 
  "personal_background_No",
  "personal_background_Yes", 
  "major_surgery_in_medical_history_No", 
  "major_surgery_in_medical_history_Yes", 
  #"hormone_therapy_imp_No", 
  #"hormone_therapy_imp_Yes", 
  #"pregnancy_imp_Male", 
  #"pregnancy_imp_No", 
  #"pregnancy_imp_Yes", 
  #"oral_contraceptive_tx_Male", 
  #"oral_contraceptive_tx_No", 
  #"oral_contraceptive_tx_NA",
  #"diabetes_mellitus_No", 
  #"diabetes_mellitus_Yes", 
  #"diabetes_mellitus_NA",
  #"dyslipidemia_No", 
  #"dyslipidemia_Yes",
  #"dyslipidemia_NA",
  "`tobacco_use_imp_Never has smoked`", 
  "`tobacco_use_imp_Former smoker`", 
  "`tobacco_use_imp_Active smoker`", 
  #"copd_No", 
  #"copd_Yes", 
  #"copd_NA",
  #"arterial_hypertension_No", 
  #"arterial_hypertension_Yes", 
  #"arterial_hypertension_NA",
  #"chronic_cardiac_insufficiency_No", 
  #"chronic_cardiac_insufficiency_Yes",
  #"chronic_cardiac_insufficiency_NA",
  #"renal_insufficiency_No", 
  #"renal_insufficiency_Yes", 
  #"renal_insufficiency_NA",
  "venous_insufficiency_imp_No", 
  "venous_insufficiency_imp_Yes",
  #"khorana_risk_score_imp_0",
  #"khorana_risk_score_imp_1",
  #"khorana_risk_score_imp_2",
  #"khorana_risk_score_imp_3", 
  #"khorana_risk_score_imp_4", 
  #"krs_category_Low risk", 
  #"krs_category_Intermediate risk",
  #"krs_category_High risk", 
  #"two_groups_krs_Low risk",
  #"two_groups_krs_High risk",
  #"tic_onco_imp_Low risk",
  #"tic_onco_imp_High risk", 
  "previous_onco_surgery_No", 
  "previous_onco_surgery_Yes", 
  "tumor_surgically_removed_No", 
...
# My.stepwise::My.stepwise.coxph(
#   T1 = "tstart", 
#   T2 = "tstop",
#   Status = "death", 
#   variable.list = stepwise_var_list,
#   data = survival_vte_tmerge_dummies
# )
```
...
table(oncoth1$data$patient_status_at_end_study_simplified, useNA = "always")
```
...
oncoth1_corr_vars = oncoth1$data %>% 
...
we see that KRS and primary tumor are collinear

GVIF Df GVIF^(1/(2*Df))
khorana_risk_score_imp                    5.309121  4        1.232049
performance_status_category_corrected_imp 1.305730  2        1.068965
tnm_stage_two_groups                      1.140239  1        1.067820
tumor_surgically_removed                  1.468709  1        1.211903
mucinous_histology_imp                    1.063991  1        1.031500
primary_tumor_simplified                  5.619393  3        1.333362
vte_event:vte_recurrence                  1.271319  3        1.040820





Results of this model are reasonable:
...
cv_cph_covariates = c(
  ...
  "histology_type_imp",
  "mucinous_histology_imp", 
  # "grade_histological_differentiation_imp", # does not hold PH assumption
  # "n_metastases",
  # "n_metastases_grouped",
  # "metastasis_adrenal_glands",
  # "metastasis_bones",
  ...
  "antiaggreg_tx_currently", #,
  # "blood_transfusion",
  # "immobilisation", # Correlated with the poor health of a patient -- somewhat redundant
  # "recent_major_surgery", 
  # "thromboprophylaxis_hospital", 
  "erythropoietin_tx"
  )
...
```{r}
# A matrix with dummies has to be created in order to do lasso Cox regression
# Missing values cause erros

# Select relevant features and ID
x_pcox = survival_vte_tmerge_stepwise[, c("id", cv_cph_covariates)]
...
# y (Surv object)
yss <- Surv(
  time = survival_vte_tmerge_stepwise$tstart, 
  time2 = survival_vte_tmerge_stepwise$tstop, 
  event = survival_vte_tmerge_stepwise$death
  )
```
...
```{r}
# Sanity check
# Check that fitting start-stop responses using glmnet with lambda = 0 matches up with coxph's result
glmnet_fit <- glmnet(x_matrix, yss, family = "cox", lambda = 0)
coxph_fit <- coxph(yss ~ x_matrix)
...
# Same as coef(glmnet_fit)
hrs_cox_glmnet = data.frame(
  "Variables" = rownames(glmnet_fit$beta), 
  "Coefficients" = as.numeric(glmnet_fit$beta[, 1])) %>% 
  filter(Coefficients != 0) %>% 
  mutate(HRs = exp(Coefficients)) %>%
  arrange(desc(HRs))
...
```{r}
print(hrs_cox_glmnet$Variables)
```



```{r}
# Train CPH model with selected variables
glmnet_l1_selected_features_cph = coxph(
  Surv(time = tstart, time2 = tstop, event = death) ~
    cluster(id) +
    tnm_stage_grouped +  
    vte_event +   
...
glmnet_l1_selected_features_cph %>%
  gtsummary::tbl_regression(exp = TRUE)
```


Why mucinous CRC is risk factor?
https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6440160/


```{r}
cox.zph(glmnet_l1_selected_features_cph)
```


```{r}
# summary(glmnet_l1_selected_features_cph)
```
...
Guide: https://cran.r-project.org/web/packages/pcoxtime/pcoxtime.pdf
...
# pcox_formula = as.formula(
#     sprintf('%s~%s', 'Surv(time = tstart, time2 = tstop, event = death)',
#             paste(cv_cph_covariates, collapse = "+ ")))

# Find optimal lambda value
# cv_cph_mortality <- pcoxtimecv(
#   formula = pcox_formula,
#   data = survival_vte_tmerge_stepwise,
#   alphas = 1, # 1 for Lasso (L1) penalty, 0 for Ridge (L2) penalty
#   lamfract = 1, # If 1, complete regularisation path is considered
#   nfolds = 3,
#   refit = TRUE,
#   quietly = TRUE,
#   seed = 2828,
...
```{r}
# str(cv_cph_mortality)
```



```{r}
# print(cv_cph_mortality)
```



```{r}
# summary(cv_cph_mortality)
```



```{r}
# coef(cv_cph_mortality)
```


```{r}
# Plot solution path depending on lambda value
# plot(cv_cph_mortality, type = "fit", xvar = "lambda", show_min_cve = TRUE)
```


```{r}
# Mean cross-validation error for various lambda-alpha combination
# extractoptimal(cv_cph_mortality, what = "cvm")
```

The chosen lambda is the one that results in the smaller mean CV error.


```{r}
# l1_penalised_cox = pcoxtime(
#   formula = pcox_formula,
#   alpha = 1,
#   lambda = cv_cph_mortality$lambda.min, # 0.01603357
#   data = survival_vte_tmerge_stepwise
# )
```
...
```{r}
# summary(l1_penalised_cox)
```


```{r}
# str(l1_penalised_cox)
```


```{r}
# print(l1_penalised_cox)
```
...
INR measures extrinsic coagulation pathway, while aPTT ratio measures the intrinsic coagulation pathway


```{r}
# Get HRs of Lasso Cox regression
# sort(exp(coef(l1_penalised_cox)[coef(l1_penalised_cox) != 0]), decreasing = TRUE)
```


```{r}
# pcoxsurvfit(l1_penalised_cox)
```


```{r}
# concordScore(l1_penalised_cox, stats = TRUE)
```


```{r}
# coef = coef(l1_penalised_cox, s = "lambda.min")
# pvalues = 2 * pnorm(-abs(coef/ l1_penalised_cox$se))
```



```{r}
# plot(varimp(l1_penalised_cox))
```


```{r}
# Train CPH model with selected variables
# l1_selected_features_cph = coxph(
#   Surv(time = tstart, time2 = tstop, event = death) ~
#     cluster(id) +
#     performance_status_category_corrected_imp + 
#     tnm_stage_grouped + 
...
# summary(l1_selected_features_cph)

# l1_selected_features_cph %>%
#   gtsummary::tbl_regression(exp = TRUE)
```


```{r}
# cox.zph(l1_selected_features_cph)
```


## RandomForest (randomForestSRC package)

It returns an error and I don't know why.

```{r}
# Build formula with all covariates
# rf_survival_formula = as.formula(
#     sprintf('%s~%s', 'Surv(time = tstart, time2 = tstop, event = death)',
#             paste(cv_cph_covariates, collapse = "+ ")))
```
...
```{r}
# rf_survival = randomForestSRC::rfsrc(
#   formula = rf_survival_formula, 
#   data = survival_vte_tmerge 
# )
```
...
all_vte_types_survival_data = oncoth1$data %>%
  select(id, 
         patient_group, 
         date_cancer_dx, 
         date_study_begins, 
...
```{r}
# %<>%
id_no_vte_only_one_type_vte = oncoth1$data %>%
...
```{r}
covariates_df_inc_sympt = oncoth1$data %>%
...
```{r}
inc_symp_td_surv_fit = survfit(Surv(
  time = tstart, 
  time2 = tstop, 
  event = death) ~ 
    first_vte_inc_symp + 
...
n_vte_no_recurrence = sum(oncoth1$data$vte_recurrence == "No VTE recurrence")
n_vte_recurrence = sum(oncoth1$data$vte_recurrence == "VTE recurrence")
...
```{r}
table(oncoth1$data$vte_recurrence, oncoth1$data$age_when_cancer_dx_65, useNA = "always")
```
...
```{r}
table(oncoth1$data$vte_recurrence, oncoth1$data$gender, useNA = "always")
```
...
```{r}
table(oncoth1$data$primary_tumor_simplified, oncoth1$data$vte_recurrence, useNA = "always")
```
