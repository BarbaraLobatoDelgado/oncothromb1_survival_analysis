...
```{r}
# Fit survival analysis model
km_fit_blood_transfusion = survfit(
  Surv(
    time = tstart, 
    time2 = tstop, 
    event = death) ~ 
...
km_fit_blood_transfusion
```


```{r}
# ...
km_pval_blood_transfusion = get_robscore_pval(survival_vte_tmerge, covariates = c("blood_transfusion"))
```
...
```{r}
blood_transf_km_plot_vte = ggsurvplot(
    fit = km_fit_blood_transfusion,
    data = survival_vte_tmerge,
    xscale = 30,
    break.time.by = 60,
    censor = FALSE,
...
```{r}
# Fit survival analysis model
immobilisation_km_fit_vte = survfit(
  Surv(
    time = tstart, 
    time2 = tstop, 
    event = death) ~ 
...
```{r}
immobilisation_km_plot_vte = ggsurvplot(
    fit = immobilisation_km_fit_vte,
    data = survival_vte_tmerge,
    xscale = 30,
    break.time.by = 60,
    censor = FALSE,
...
# Add p-value
immobilisation_km_plot_vte$plot = 
  immobilisation_km_plot_vte$plot + 
  annotate(
    geom = "text", 
    x = 26,
...
```{r}
# Fit survival analysis model
recent_major_surgery_km_fit_vte = survfit(
  Surv(
    time = tstart, 
    time2 = tstop, 
    event = death) ~ 
...
recent_major_surgery_km_fit_vte
```
...
```{r}
recent_major_surgery_km_plot_vte = ggsurvplot(
    fit = recent_major_surgery_km_fit_vte,
    data = survival_vte_tmerge,
    xscale = 30,
    break.time.by = 60,
    censor = FALSE,
...
# Add p-value
recent_major_surgery_km_plot_vte$plot = 
  recent_major_surgery_km_plot_vte$plot + 
  annotate(
    geom = "text", 
    x = 26,
...
recent_major_surgery_km_plot_vte
...
```{r}
# Fit survival analysis model
thromboprophylaxis_hospital_km_fit_vte = survfit(
  Surv(
    time = tstart, 
    time2 = tstop, 
    event = death) ~ 
...
thromboprophylaxis_hospital_km_fit_vte
```
...
```{r}
thromboprophylaxis_hospital_km_plot_vte = 
  ggsurvplot(
    fit = thromboprophylaxis_hospital_km_fit_vte,
    data = survival_vte_tmerge,
    xscale = 30,
    break.time.by = 60,
    censor = FALSE,
...
# Add p-value
thromboprophylaxis_hospital_km_plot_vte$plot = 
  thromboprophylaxis_hospital_km_plot_vte$plot + 
  annotate(
    geom = "text", 
    x = 26,
...
```{r}
# Fit survival analysis model
erythropoietin_tx_km_fit_vte = survfit(
  Surv(
    time = tstart, 
    time2 = tstop, 
    event = death) ~ 
...
```{r}
erythropoietin_tx_km_plot_vte = 
  ggsurvplot(
    fit = erythropoietin_tx_km_fit_vte,
    data = survival_vte_tmerge,
    xscale = 30,
    break.time.by = 60,
    censor = FALSE,
...
# Add p-value
erythropoietin_tx_km_plot_vte$plot = 
  erythropoietin_tx_km_plot_vte$plot + 
  annotate(
    geom = "text", 
    x = 26,
...
```{r}
# Fit survival analysis model
stratified_stage_erythropoietin_tx_km_fit_vte = survfit(
  Surv(
    time = tstart, 
    time2 = tstop, 
    event = death) ~ 
...
stratified_stage_erythropoietin_tx_km_fit_vte
```
...
```{r}
stratified_stage_erythropoietin_tx_km_plot_vte = 
  ggsurvplot(
    fit = stratified_stage_erythropoietin_tx_km_fit_vte,
    data = survival_vte_tmerge,
    xscale = 30,
    break.time.by = 60,
    censor = FALSE,
...
# Add p-value
stratified_stage_erythropoietin_tx_km_plot_vte$plot = 
  stratified_stage_erythropoietin_tx_km_plot_vte$plot + 
  annotate(
    geom = "text", 
    x = 26,
...
```{r}
survival_vte_tmerge_tt = survival_vte_tmerge %>%
  select(-c(id, contains("tu_"))) %>%
  fastDummies::dummy_cols(., remove_first_dummy = TRUE, remove_selected_columns = TRUE)

# Variance per feature
variance_per_feature = sapply(survival_vte_tmerge_tt, function (x) var(x, na.rm = TRUE))
variance_per_feature[order(variance_per_feature)]
```
...
```{r}
# # Train stepwise feature selection Cox Proportional Hazards regression for event death
# StepReg::stepwiseCox(
...
#     # major_surgery_in_medical_history + 
#     # hormone_therapy + 
#     # pregnancy + # Low variance
#     # oral_contraceptive_tx + # Low variance
#     diabetes_mellitus +
#     dyslipidemia +
#     tobacco_use_imp +
#     # tobacco_use_imp_grouped +
...
for (i in 1:10) {
  ...
  # selected_variables = as.vector(unlist(stepwise_model$`Selected Variables`))
  selected_variables = as.vector(unlist(stepwise_model$`Summary of coefficients for the selected model with Surv(time = tstart, time2 = tstop, event = death) under bidirection and AIC `$Variable))
