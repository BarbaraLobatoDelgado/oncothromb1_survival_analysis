...
# Define lambda grid
lambda1_grid <- c(0.1, 1, 5, 10, 25, 50, 75, 80, 100)
# lambda1_grid <- 1
...
summary_regpath_train <- regpath_results %>%
  group_by(lambda1) %>%
  summarise(
    mean_cindex_train = mean(cindex_train, na.rm = TRUE), 
    .groups = "drop"
  )
summary_regpath_train
```
...
```{r}
# Select variables
training_set <- process_data(training_set, cols2select)
test_set <- process_data(test_set, cols2select)
...
# 2) Apply to both train & valid
training_set_transf <- predict(preprocess, training_set)
test_set_transf <- predict(preprocess, test_set)
```
