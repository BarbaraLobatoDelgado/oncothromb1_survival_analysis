...
stopifnot(dim(covariates_scaled)[0] == length(response))
```
...
stopifnot(dim(training_set)[0] + dim(test_set)[0] == covariates_scaled)
...
for (i in 1:kfolds) {       # each i is the number of a fold
  ...
  validation_fold <- training_set[training_set_with_ids$id %in% validation_ids, ]
  # Scale data
  validation_fold <- scale(validation_fold)
  
  # # Separate target from data
  # y_validation_fold <- y_training
  # validation_fold <- validation_fold %>% select(-death)
  
  # Get the training fold
  training_fold <- training_set[!(training_set_with_ids$id %in% validation_ids), ]
  # Scale data
  training_fold <- scale(training_fold)
  ...
  # Check dimensions
  stopifnot(dim(training_fold)[0] + dim(validation_fold)[0] == dim(training_set)[0])
