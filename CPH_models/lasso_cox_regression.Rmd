...
stopifnot(
  nrow(training_set) + nrow(test_set) == nrow(oncoth1_td_dataset),
...
```{r}
# # --- 1) Attach fold to training_set ---------------------------------------
# # training_set: your 80% train-only df with id, tstart, tstop, death, predictors
# # fold_map: tibble with patient_id, fold (1…kfolds)
# DF <- training_set %>%
#   rename(patient_id = id) %>%
#   left_join(fold_map, by = "patient_id") %>%
#   select(patient_id, fold, tstart, tstop, death, all_of(cols2select))
# 
# lambda1_grid <- c(0.001, 0.01, 0.1, 1, 5, 10)
# 
# # --- 2) Prepare results tibble --------------------------------------------
...
#   
#   # Split DF
#   train_df <- filter(DF, fold != fold_i)
#   valid_df <- filter(DF, fold == fold_i)
#   
#   # Sanity checks
#   stopifnot(
#     nrow(train_df) + nrow(valid_df) == nrow(DF),
#     length(unique(train_df$patient_id)) + length(unique(valid_df$patient_id)) ==
#       length(unique(DF$patient_id))
#   )
#   
#   # Build X and y
#   X_train <- process_data(train_df, cols2select)
#   X_valid <- process_data(valid_df, cols2select)
#   stopifnot(!anyNA(X_train), !anyNA(X_valid))
#   
#   y_train <- with(train_df, Surv(tstart, tstop, death))
#   y_valid <- with(valid_df, Surv(tstart, tstop, death))
#   
#   # Fit penalized Cox
#   fit_cv <- penalized(
#     response    = y_train,
...
#   )
#   
#   # Linear predictor and C-index
#   beta_valid <- coef(fit_cv, "all", standardize = TRUE)
#   stopifnot(all(names(beta_valid) == colnames(X_valid)))
#   
#   lp_valid <- as.numeric(X_valid %*% beta_valid)
#   regpath_results$cindex[row_idx] <- concordance(y_valid ~ lp_valid)$concordance
#   
#   # Record selected variables (non-zero)
#   nz <- coef(fit_cv)
#   regpath_results$selected_vars[[row_idx]] <- names(nz)[nz != 0]
# }
```
...
for (row_idx in seq_len(nrow(regpath_results))) {
  ...
  # # Split by fold (as before)
  # train_df <- filter(training_set, fold != fold_i)
  # valid_df <- filter(training_set, fold == fold_i)
  # 
  # Build unscaled X’s
