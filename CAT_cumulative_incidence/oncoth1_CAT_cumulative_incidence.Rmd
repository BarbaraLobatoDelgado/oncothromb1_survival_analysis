...
get_cumulative_incidence = function(fit_summary, event_code = 1){
  ...
  print(paste("12-month CAT cumulative incidence for ONCOTHROMB1 cohort is", round(1 - cuminc_12month_stats$surv, digits = 4) * 100, "% (CI", round(1 - cuminc_12month_stats$upper, digits = 4) * 100, "% -", round(1 - cuminc_12month_stats$lower, digits = 4) * 100, "%)"))
  
  
  # 18-month CAT cumulative incidence
  cuminc_18month_stats <- fit_summary %>% 
    filter(n.event == event_code) %>%
    filter(time >= (eighteen_months_in_days - window_size) & time <= (eighteen_months_in_days + window_size))
  
  print(paste("18-month CAT cumulative incidence for ONCOTHROMB1 cohort is", round(1 - cuminc_18month_stats$surv, digits = 4) * 100, "% (CI", round(1 - cuminc_18month_stats$upper, digits = 4) * 100, "% -", round(1 - cuminc_18month_stats$lower, digits = 4) * 100, "%)"))
}
```
...
# 12-month CAT cumulative incidence
cat_cuminc_12month_stats <- cat_fit_summary %>% 
  filter(n.event == 1) %>%
  slice(which.min(abs(time - twelve_months_in_days)))
# cat_cuminc_12month_stats

print(paste("12-month CAT cumulative incidence for ONCOTHROMB1 cohort is", round(1 - cat_cuminc_12month_stats$surv, digits = 4), "(CI", round(1 - cat_cuminc_12month_stats$upper, digits = 4), "-", round(1 - cat_cuminc_12month_stats$lower, digits = 4), ")"))


# 18-month CAT cumulative incidence
cat_cuminc_18month_stats <- cat_fit_summary %>% 
  filter(n.event == 1) %>%
  filter(time >= (eighteen_months_in_days - window_size) & time <= (eighteen_months_in_days + window_size))
# cat_cuminc_18month_stats

print(paste("18-month CAT cumulative incidence for ONCOTHROMB1 cohort is", round(1 - cat_cuminc_18month_stats$surv, digits = 4), "(CI", round(1 - cat_cuminc_18month_stats$upper, digits = 4), "-", round(1 - cat_cuminc_18month_stats$lower, digits = 4), ")"))
```
...
For colorectal cancer patients, the 12-month CAT cumulative incidence is 11.61% (CI ??) and the 18-month cumulative incidence is 12.94% (CI ??). 
...
# Thirty patients are excluded for the cumulative incidence estimation
# However, the rest must be accounted for
stopifnot(n_cat_colorectal_patients + sum(cat_cuminc_data$n_appointment_patient_became_case == "Beginning of study") == sum(cat_cuminc_data$patient_group == "Case"))
```
