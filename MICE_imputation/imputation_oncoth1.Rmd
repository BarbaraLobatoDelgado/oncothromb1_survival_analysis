...
data_for_imputation %>% 
  select(-c(id)) %>% # Remove ID
  # Transform some factors into numeric
  mutate(c_across(performance_status_category_corrected), ~ as.numeric(.x)) %>%
  # Filter(is.factor, .) %>% # Get only factors
  model.matrix(~0+., data = .) %>% # One-hot encoding (no intercept)
  cor(use="pairwise.complete.obs") %>% 
  ggcorrplot::ggcorrplot(show.diag = F, type="lower", lab=TRUE, lab_size=2, tl.cex = 10)
```
...
model.matrix(~0+., data=Filter(is.numeric, data_for_imputation))  %>% # one-hot encoding
  cor(use="pairwise.complete.obs") %>% 
  ggcorrplot::ggcorrplot(show.diag = F, type="lower", lab=TRUE, lab_size=2, tl.cex = 10)
```
