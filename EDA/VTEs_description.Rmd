...
## Number/proportion of VTE events per month 
...
```{r}
# Sanity check
stopifnot(sum(VTE_events_by_month$n_VTEs) == 120)
```
...
VTE_events_by_month_corrected[3, 2:3] = colSums(VTE_events_by_month[1:3, 2:3])
VTE_events_by_month_corrected = VTE_events_by_month_corrected[-c(1:2), ]
```
...
ggplot(
  ...
  aes(x = VTE_events_by_month, y = percentage)) +
...
```{r}
patients_cat_by_month = VTE_data %>% 
  select(
    id, 
    n_all_vte,
