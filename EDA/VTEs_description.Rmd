...
tu_vte_data = oncoth1$data %>%
  select(id, 
         ...
         tu_5th_vte_all_from_cancer_diagnosis_months,
         # Corrected times
...
stopifnot(dim(tu_vte_data)[1] == n_patients_vte)
```


## Number/proportion of CAT occurrence per month 

I am gonna ignore the distinction of incidental and symptomatic for now.  

```{r}
# Concatenate all months in which a VTE happens
all_vtes_freq_by_month = c(
  tu_vte_data$tu_1st_vte_all_from_cancer_diagnosis_months, 
  tu_vte_data$tu_2nd_vte_all_from_cancer_diagnosis_months, 
  tu_vte_data$tu_3rd_vte_all_from_cancer_diagnosis_months,
  tu_vte_data$tu_4th_vte_all_from_cancer_diagnosis_months,
  tu_vte_data$tu_5th_vte_all_from_cancer_diagnosis_months,
  na.rm = TRUE
)

all_vtes_freq_by_month = as.data.frame(all_vtes_freq_by_month)

all_vtes_freq_by_month %<>%
  count(all_vtes_freq_by_month) %>%
  na.omit() %>%
...
```{r}
# Collapsing infomation of months -2, -1 and 0 into one
all_vtes_freq_by_month_corrected = all_vtes_freq_by_month
all_vtes_freq_by_month_corrected[3, 2:3] = colSums(all_vtes_freq_by_month[1:3, 2:3])
all_vtes_freq_by_month_corrected = all_vtes_freq_by_month_corrected[-c(1:2), ]
```


```{r}
# jpeg("../graphs/paper/barplot_CAT_ocurrence_per_month.jpeg", width = 6.5, height = 4, units = 'in', res = 700)

# Barplot
# Patients affected by CAT over time
ggplot(
  data = all_vtes_freq_by_month, 
  aes(x = all_vtes_freq_by_month, y = percentage)) +
  geom_bar(stat = "identity", width = 0.8, color = "grey", fill = "lightgrey") +
  geom_line() +
  geom_point() +
  ggtitle("CAT occurrence with respect to cancer diagnosis date") + 
  xlab("Time (months)") + 
  ylab("CAT event occurrence") + 
...
# jpeg("../graphs/paper/barplot_CAT_ocurrence_per_month_corrected.jpeg", width = 6.5, height = 4, units = 'in', res = 700)

# Barplot
# Patients affected by CAT over time
ggplot(
  data = all_vtes_freq_by_month_corrected, 
  aes(x = all_vtes_freq_by_month, y = percentage)) +
  geom_bar(stat = "identity", width = 0.8, color = "grey", fill = "lightgrey") +
  geom_line() +
  geom_point() +
...
## Number/proportion of patients affected by CAT per month 

```{r}
patients_cat_by_month = tu_vte_data %>% 
  select(
    id, 
    n_all_vte,
  ...
  mutate(tu_1st_vte_all_from_cancer_diagnosis_months = case_when(
    ...
    # tu_1st_vte_all_from_cancer_diagnosis_days <= 0 ~ 0,
  ...
  # For some reason, 31 patients are counted but there are 30 patients considered with VTE
  # prior or concomitant
  mutate(n = case_when(
    month_VTE_occurrence == 0 ~ 30, 
    TRUE ~ n
  )) %>% 
...
```{r}
# jpeg("../graphs/paper/barplot_patients_affected_by_CAT_per_month.jpeg", width = 6.5, height = 4, units = 'in', res = 700)

# Barplot
# Patients affected by CAT over time
ggplot(
  data = patients_cat_by_month, 
...
## Frequency of VTE episode's type by location


```{r}
# How many combinations of VTE events by location exist?
vte_loc = c("CRT", "DVT", "SVT", "PTE", "VT")
combinations = combn(vte_loc, 2)
print(combinations)
```
...
stopifnot(vte_by_location$n_all_vte == vte_by_location$n_total_crt_all + vte_by_location$n_total_dvt_all + vte_by_location$n_total_svt_all + vte_by_location$n_total_pte_all + vte_by_location$n_total_visceral_thromb_all)
...
n_vtes_oncoth1 = sum(vte_by_location$n_all_vte)
```


```{r}
print(paste("A total of", nrow(vte_by_location), "patients had", n_vtes_oncoth1, "VTEs"))
```


```{r}
# Count frequency of patients with one or more types of VTE by location
summary_n_patients_with_vte_by_location = vte_by_location %>%
  select(
    n_crt, 
    n_dvt, 
    n_vt, 
...
stopifnot(sum(summary_n_patients_with_vte_by_location$freq_abs) == n_patients_vte)
```


```{r}
# jpeg("../graphs/paper/barplot_vte_by_location_corrected.jpeg", width = 6.5, height = 4, units = 'in', res = 700)
# —
...
ggplot(
  data = summary_n_patients_with_vte_by_location, 
...
## Incidental/symptomatic VTE episodes by cancer type

```{r}
# Dataframe with incidental/symptomatic VTEs
vte_type_sympt = oncoth1$data %>% 
  filter(patient_group == "Case") %>%
  select(
    id, 
    primary_tumor_simplified,
...
# count the number of times a string appears in multiple columns
vte_type_sympt_count = vte_type_sympt %>%
  rowwise() %>%
...
# xx = vte_type_sympt_count[!(vte_type_sympt_count$n_all_vte == vte_type_sympt_count$count_inc + vte_type_sympt_count$count_sympt), ]
...
# Patient id = 258 had 1 VT of unknown symptomatology in 2015-10-22
# stopifnot(vte_type_sympt_count$n_all_vte == vte_type_sympt_count$count_inc + vte_type_sympt_count$count_sympt)
```


```{r}
# vte_symptoms_df = vte_type_sympt_count %>%
#   select(id, primary_tumor_simplified, count_inc, count_sympt)

# Number of incidental and symptomatic events per cancer type
vte_symptoms_per_cancer_type = vte_type_sympt_count %>%
  select(id, primary_tumor_simplified, count_inc, count_sympt) %>%
...
vte_symptoms_per_cancer_type %<>%
  bind_rows(
    data.frame(primary_tumor_simplified = "All",
               total_count_inc = sum(vte_symptoms_per_cancer_type$total_count_inc),
               total_count_sympt = sum(vte_symptoms_per_cancer_type$total_count_sympt))
  ) %>%
...
vte_symptoms_per_cancer_type %<>%
  pivot_longer(cols = contains("freq"), 
               names_to = "vte_type") %>%
  mutate(vte_type_sympt = ifelse(grepl("_inc", vte_type), "Incidental", "Symptomatic")) %>%
  ...
  group_by(primary_tumor_simplified, vte_type_sympt) %>%
...
```{r}
ggplot(
  data = vte_symptoms_per_cancer_type, 
  aes(x = primary_tumor_simplified, y = abs_freq, fill = vte_type_sympt)) + 
...
ggstatsplot::ggbarstats(
  data = vte_symptoms_per_cancer_type,
  x = primary_tumor_simplified,
  y = abs_freq,
  fill = vte_type_sympt,
  counts = abs_freq,          # Specify the frequency column
  paired = TRUE,              # This indicates that the bars are paired within the levels of x
  package = "ggsci",          # Optional: Use a color palette from the ggsci package
  palette = "default_jco",    # Optional: Set a specific palette
  label = "p.value",          # Adds the p-values to the plot
  xlab = "Primary Tumor and VTE Type", 
  ylab = "Frequency",
  ggtheme = ggplot2::theme_minimal(),  # Apply a minimal theme
  legend.title = "VTE Type"
) + 
...
cat_prior_or_concomitant_by_cancer_type = tu_vte_data %>%
  group_by(primary_tumor_simplified) %>%
...
stopifnot(
  sum(cat_prior_or_concomitant_by_cancer_type$n_vte_prior_or_concomitant) ==
    ...
    sum(tu_vte_data$tu_1st_vte_all_from_cancer_diagnosis_months <= 0, na.rm = TRUE) +
    sum(tu_vte_data$tu_2nd_vte_all_from_cancer_diagnosis_months <= 0, na.rm = TRUE) + 
    sum(tu_vte_data$tu_3rd_vte_all_from_cancer_diagnosis_months <= 0, na.rm = TRUE) + 
    sum(tu_vte_data$tu_4th_vte_all_from_cancer_diagnosis_months<= 0, na.rm = TRUE)
  )
...
cat_recurrences_by_cancer_type = tu_vte_data %>%
  group_by(primary_tumor_simplified) %>%
...
#     # Count non-NA 
#     sum(!is.na(tu_vte_data$tu_1st_rec_vte_from_cancer_diagnosis_months), na.rm = TRUE)
#   )
...
cat_during_study_by_cancer_type = tu_vte_data %>%
  group_by(primary_tumor_simplified) %>%
...
#     sum(tu_vte_data$tu_3rd_vte_all_from_cancer_diagnosis_months > 0, na.rm = TRUE) +
#     sum(tu_vte_data$tu_4th_vte_all_from_cancer_diagnosis_months > 0, na.rm = TRUE) +
#     sum(tu_vte_data$tu_5th_vte_all_from_cancer_diagnosis_months > 0, na.rm = TRUE) -
#     sum(tu_vte_data$n_total_vte_recurrence)
#   )
...
```{r}
# Count number of VTE episodes prior or concomitant to cancer diagnosis by cancer type
cat_prior_or_concomitant_by_cancer_type_patients = tu_vte_data %>%
  group_by(primary_tumor_simplified) %>%
...
```{r}
# Count number of VTE episodes that are recurrences by cancer type
cat_recurrences_by_cancer_type_patients = tu_vte_data %>%
  group_by(primary_tumor_simplified) %>%
...
```{r}
# Count number of VTE episodes during study by cancer type
cat_during_study_by_cancer_type_patients = tu_vte_data %>%
  group_by(primary_tumor_simplified) %>%
...
cat_inc_symp = vte_symptoms_per_cancer_type %>% 
  select(-rel_freq) %>%
  pivot_wider(names_from = vte_type_sympt, values_from = abs_freq, values_fill = 0) %>%
  ungroup() %>%
  as.data.frame() 

rownames(cat_inc_symp) = cat_inc_symp$primary_tumor_simplified
...
x <- c()
for (row in rownames(cat_inc_symp)) {
  for (col in colnames(cat_inc_symp)) {
    x <- rbind(x, matrix(rep(c(row, col), cat_inc_symp[row, col]), ncol = 2, byrow = TRUE))
  }
}
df1 <- as.data.frame(x)
colnames(df1) <- c("cancer_type", "vte_type")
df1
```
...
test <- fisher.test(table(df1))

# Do barplot with several statistical test
ggbarstats(
  df1, vte_type, cancer_type,
...
# yy = left_join(
#   vte_by_location, 
#   vte_type_sympt_count, 
#   by = c("id", "primary_tumor_simplified", "n_all_vte")
#   ) # %>%
  
cancer_vte_loc = vte_by_location %>%
...
vte_type_sympt_count_long <- vte_type_sympt_count %>%
  group_by(primary_tumor_simplified) %>%
  pivot_longer(
    cols = contains("type"), 
...
count_vte_loc_sympt <- vte_type_sympt_count_long %>%
...
patients_vte_loc_sympt_cancer_type = vte_type_sympt_count_long %>%
