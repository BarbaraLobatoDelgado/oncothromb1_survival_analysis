...
x <- c()
for (row in rownames(contigency_table_VTE_symptomatology_by_cancer_type)) {
  for (col in colnames(contigency_table_VTE_symptomatology_by_cancer_type)) {
    x <- rbind(x, matrix(rep(c(row, col), contigency_table_VTE_symptomatology_by_cancer_type[row, col]), ncol = 2, byrow = TRUE))
  }
}
