...
x <- c()
for (row in rownames(VTE_location_by_cancer_type)) {
  for (col in colnames(VTE_location_by_cancer_type)) {
    x <- rbind(x, matrix(rep(c(row, col), VTE_location_by_cancer_type[row, col]), ncol = 2, byrow = TRUE))
  }
}
VTE_location_by_cancer_type_longer <- as.data.frame(x)
colnames(VTE_location_by_cancer_type_longer) <- c("cancer_type", "vte_type")
VTE_location_by_cancer_type_longer
rm(x)
```
